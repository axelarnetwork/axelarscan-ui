...
function Info({ data, address, rewards, cumulativeRewards }) {
  ...
  const { bonding_state, authorization_state, weight } = {
    ...Object.values({ ...verifiersByChain })
      .flatMap(d => toArray(d?.addresses))
      .find(d => equalsIgnoreCase(d.address, address)),
  };
          ...
          {supportedChains && (
            <>
              <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 flex flex-col space-y-2 text-sm leading-6 text-zinc-700 dark:text-zinc-300 sm:col-span-2 sm:mt-0">
                  <Tag
                    className={clsx(
                      'w-fit',
                    ...
                    <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                      <thead className="sticky top-0 z-10 bg-white dark:bg-zinc-900">
                        <tr className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                          ...
                          <th
                            scope="col"
                            className="whitespace-nowrap px-3 py-2.5 pl-3 pr-4 text-right sm:pr-3"
                          >
                            Payout at
                          </th>
                        </tr>
...
function Signs({ data }) {
  ...
  const totalSigned = toArray(data).filter(
    d => typeof d.sign === 'boolean' && d.sign
  ).length;
      ...
      <div className="my-2.5 flex flex-col gap-y-2">
        <div className="flex justify-between gap-x-4 pr-1">
          ...
          <div className="flex flex-col items-end">
            <Number
              value={
                (data.filter(d => typeof d.sign === 'boolean').length * 100) /
                data.length
              }
              suffix="%"
              className="text-sm font-semibold leading-6 text-zinc-900 dark:text-zinc-100"
