                ...
                {data.map((d, i) => (
                  <tr key={i} className="align-top text-zinc-400 dark:text-zinc-500 text-sm">
                    ...
                    <td className="table-cell pl-3 pr-4 sm:pr-0 py-4 text-left">
                      <div className={clsx('grid grid-cols-2 lg:grid-cols-3 gap-x-2 gap-y-1', Object.entries({ ...verifiersByChain }).filter(([k, v]) => vmChains.findIndex(d => d.id === k) < 0 && toArray(v.addresses).length > 0).length > 0 ? 'min-w-md lg:min-w-56 max-w-4xl' : 'min-w-56 max-w-3xl')}>
                        {_.concat(vmChains, Object.entries({ ...verifiersByChain }).filter(([k, v]) => vmChains.findIndex(d => d.id === k) < 0 && toArray(v.addresses).length > 0).map(([k, v]) => k)).map(c => {
                          const { id, name, image } = { ...(typeof c === 'string' ? { id: c, name: c } : c) }
                          const { votes, total, total_polls } = { ...d.votes.chains[id] }
                          const { signs, total_proofs } = { ...d.signs.chains[id] }
                          const isSupported = d.supportedChains.includes(id)
