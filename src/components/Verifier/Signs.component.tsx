...
export function Signs({ data }: SignsProps) {
  ...
  const totalSigned = entries.filter(
  ...
  const participationRate =
    (entries.filter(d => typeof d.sign === 'boolean').length * 100) /
    data.length;
  const summaryPrefix = `${Object.keys(totalSigns).length > 1 ? '(' : ''}${Object.entries(
    totalSigns
  )
    ...
    <div className={styles.sectionWrapper}>
      ...
      <div className={styles.blockGrid}>
        {data.map((d, i) => {
          ...
          const tooltipContent = d.session_id
