...
export function Votes({ data }: VotesProps) {
  ...
  const totalY = entries.filter(d => typeof d.vote === 'boolean' && d.vote).length;
  const totalN = entries.filter(d => typeof d.vote === 'boolean' && !d.vote).length;
  const totalUN = entries.filter(d => typeof d.vote !== 'boolean').length;
  const totalVotes = Object.fromEntries(
    Object.entries({ Y: totalY, N: totalN, UN: totalUN }).filter(([_k, v]) => v || _k === 'Y')
  );

  const participationRate = (entries.filter(d => typeof d.vote === 'boolean').length * 100) / data.length;
  const summaryPrefix = `${Object.keys(totalVotes).length > 1 ? '(' : ''}${Object.entries(totalVotes)
    ...
    <div className={styles.sectionWrapper}>
      <div className={styles.sectionHeader}>
        <div className={styles.sectionHeaderLeft}>
          ...
          <p className={styles.sectionSubtitle}>Latest {numberFormat(SIZE, '0,0')} Polls</p>
        </div>
      ...
      <div className={styles.blockGrid}>
        {data.map((d, i) => {
          ...
          const tooltipContent = d.poll_id ? `Poll ID: ${d.poll_id} (${name})` : numberFormat(d.height, '0,0');
