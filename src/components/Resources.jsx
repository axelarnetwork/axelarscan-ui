...
function Asset({ data, focusID, onFocus }) {
  ...
  const {
    ...
    ibc_denom,
    symbol: tokenSymbol,
  } = { ...addresses.find(d => d.chain === chain) };
  ...
  return (
    <li>
      <div className="relative rounded-2xl bg-zinc-50/75 p-6 dark:bg-zinc-800/25">
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="font-display text-xl font-medium">{data.name}</span>
          {data.decimals > 0 && (
            <span className="mt-0.5 whitespace-nowrap text-sm font-normal text-zinc-400 dark:text-zinc-500">
                    ...
                    >
                      ...
                      >
                        <Image
                          ...
                          className={clsx(
                            'rounded-full',
                            focusID === asset && chain === chainSelected
          ...
          {chain && (
            <div className="flex flex-col gap-y-3">
              <div className="flex items-center justify-between gap-x-2">
                <Tag className="uppercase">{chain}</Tag>
                {chain_type === 'evm' && (
...
export function Resources({ resource }) {
  ...
  const filter = (resource, params) => {
    ...
    switch (resource) {
      ...
      case 'assets':
        return _.concat(
          toArray((!type || type === 'gateway') && assets)
            .filter(d => !chain || d.addresses?.[chain])
            .filter(
              d =>
                !input ||
                includesSomePatterns(
      ...
      <Container className="flex flex-col gap-y-8 sm:mt-8 sm:gap-y-12">
        <div className="flex flex-col gap-y-4 sm:flex-row sm:items-center sm:justify-between sm:gap-x-2 sm:gap-y-0">
          ...
          <div className="flex max-w-sm flex-col items-start gap-y-2 sm:items-end">
            ...
            {attributes.length > 0 && (
              <div className="mt-2 flex flex-1 flex-col justify-between gap-y-2">
                {attributes.map((d, i) => (
                  <div key={i} className="flex items-center gap-x-4">
                    <label
                      htmlFor={d.name}
                      className="text-sm font-medium leading-6 text-zinc-900 dark:text-zinc-500"
                    >
                      {d.label}
                    </label>
                    <div className="w-48">
                      {d.type === 'select' ? (
                        <Listbox
                          ...
                          onChange={v => {
                            router.push(
                              `/resources/${resource}${getQueryString({ ...params, [d.name]: d.multiple ? v.join(',') : v })}`
                            );
                          }}
                          multiple={d.multiple}
                        >
                                      ...
                                      {selectedValue.length === 0 ? (
                                        ...
                                        selectedValue.map((v, j) => (
                                          <div
                                            ...
                                            className="my-1 mr-2 flex h-6 min-w-fit items-center rounded-xl bg-zinc-100 px-2.5 py-1 text-zinc-900 dark:text-zinc-100"
                                          >
                                            {v.title}
                                          </div>
                                        ))
