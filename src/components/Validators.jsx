...
export function Validators({ status }) {
  ...
  useEffect(() => {
    if (chains) {
      setEVMChains(
        chains.filter(
          d => d.chain_type === 'evm' && d.gateway?.address && !d.no_inflation
        )
      );
    }
  }, [setEVMChains, chains]);
      ...
      const _data = _.orderBy(
        validators.map(d => {
          ...
          const supportedChains = Object.entries({ ...maintainers })
            .filter(([k, v]) => find(d.operator_address, v))
            .map(([k, v]) => k);

          const inflation = toFixed(
            (d.uptime / 100) * toNumber(tendermintInflationRate) +
              (isNumber(d.heartbeats_uptime) ? d.heartbeats_uptime / 100 : 1) *
              ...
              toNumber(externalChainVotingInflationRate) *
                _.sum(
                  supportedChains.map(c => {
                    ...
                    return (
                      1 -
                      (total_polls ? (total_polls - total) / total_polls : 0)
                    );
                  })
                ),
            6
          ...
          return {
            ...
            apr:
              (inflation *
                100 *
                formatUnits(bankSupply?.amount, 6) *
                (1 - toNumber(communityTax)) *
                (1 - toNumber(rate))) /
              formatUnits(stakingPool?.bonded_tokens, 6),
            supportedChains,
            votes: d.votes && {
              ...d.votes,
                ...
                {(!status || status === 'active') && (
                  <>
                    ...
                    <Link
                      ...
                      className="text-sm font-semibold text-blue-600 dark:text-blue-500"
                    >
                      How to stake AXL
                    </Link>
                  </>
            ...
            <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
              <thead className="sticky top-0 z-10 bg-white dark:bg-zinc-900">
                <tr className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                  <th
                    scope="col"
                    onClick={() => (order[0] !== 'tokens' ? orderBy('') : {})}
                    className={clsx(
                      'py-3.5 pl-4 pr-3 text-left sm:pl-0',
                      order[0] !== 'tokens' && 'cursor-pointer'
                    )}
                  >
                    #
                  </th>
                  ...
                  <th
                    ...
                    className="cursor-pointer whitespace-nowrap px-3 py-3.5 text-left"
                  >
                    <div className="flex items-center">
                      <span>
                  ...
                  <th
                    ...
                    className="hidden cursor-pointer px-3 py-3.5 text-left sm:table-cell"
                  >
                    <div className="flex items-center">
                      <span>Uptime</span>
                      {order[0] === 'uptime' && (
                ...
                {filter(status).map((d, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-4 text-left">
                        {isNumber(d.tokens) && (
                          <div className="grid max-w-32 gap-y-2">
                      ...
                      <td className="hidden px-3 py-4 text-left sm:table-cell">
                        <div className="my-0.5 grid min-w-24 max-w-24 gap-y-2">
                          {isNumber(d.uptime) && (
                            <ProgressBar
                              value={d.uptime}
                              className={clsx(
                      ...
                      <td className="table-cell py-4 pl-3 pr-4 text-left sm:pr-0">
                        <div className="grid min-w-56 grid-cols-2 gap-x-2 gap-y-1 lg:grid-cols-3">
                          {toArray(chains)
                            .filter(
                              c => c.chain_type === 'evm' && !c.deprecated
                            )
                            .map(c => {
