      ...
      toArray(
        _.concat(
          toArray<TransactionMessage>(messages).map(d =>
            camel(
              isString(d.msg) ? (d.msg as string) : Object.keys({ ...d.msg })[0]
            )
          ),
          toArray<TransactionMessage>(messages).map(
            d => d.inner_message?.['@type'] as string | undefined
          ),
          normalizeEvents(data as Parameters<typeof normalizeEvents>[0])
            ...
            .map(
              e =>
                getAttributeValue(
                  e.attributes as CosmosAttributeLike[],
                  'action'
                ) as string | undefined
            ),
...
export const getActivities = (
  ...
  if (
    includesSomePatterns(
      messages
        .map((d: TransactionMessage) => d['@type'])
    ...
    result = toArray<TransactionActivity>(
      messages.flatMap(
        (d: TransactionMessage, i: number): TransactionActivity[] => {
          ...
          const msgEvents = normalizedEvents.filter(
            e =>
              getMsgIndexFromEvent(
                e as Parameters<typeof getMsgIndexFromEvent>[0]
              ) === i
          );
          ...
          const activity: TransactionActivity = {
            ...
            source_channel,
            destination_channel,
            timeout_timestamp: formatUnits(
              String(d.timeout_timestamp ?? '0'),
              6
            ),
          };
          ...
          if (
            amountArray &&
            toArray<TransactionAmount>(amountArray).length > 0
          ) {
            return toArray<TransactionAmount>(amountArray).map(
  ...
  // Confirm* / Vote-related messages
    ...
    includesSomePatterns(
      messages.flatMap((d: TransactionMessage) =>
        toArray([d['@type'], d.inner_message?.['@type']])
      ),
      [
        'ConfirmDeposit',
        'ConfirmTokenRequest',
    ...
    result = toArray<TransactionActivity>(
      messages
        .flatMap((d: TransactionMessage) => {
          let { chain, asset, tx_id } = { ...d };
          const innerMessage = d.inner_message as
            | {
                poll_id?: string;
                vote?: {
                  chain?: string;
          ...
          const status = (vote?.events?.[0]?.status || d.status) as
            | string
            | undefined;
  ...
  if (toArray<TransactionActivity>(result).length < 1) {
    result = normalizeEvents(
      ...
      if (find(cosmosEvent.type, ['delegate', 'unbond', 'transfer'])) {
        ...
        toArray<CosmosAttributeLike>(cosmosEvent.attributes).forEach(a => {
          ...
          if (
            key ===
            (toArray<CosmosAttributeLike>(cosmosEvent.attributes).findIndex(
              a => a.key === 'denom'
            ) > -1
              ? 'denom'
              : 'amount')
            ...
            switch (cosmosEvent.type.toLowerCase()) {
              case 'delegate':
                ...
                break;
              case 'unbond':
                _e.recipient = (_e.recipient || delegator_address) as
                  | string
                  | undefined;
                break;
              default:
        ...
        ...(
          ...
          toArray<CosmosAttributeLike>(cosmosEvent.attributes).map(
            ({ key, value }) => {
              ...
              let { symbol, amount } = { ...attribute } as {
                symbol?: string;
    ...
    const resultTypes = toArray<TransactionActivity>(result)
      .map(e => e.type)
      .filter((t): t is string => !!t);

    if (includesSomePatterns(resultTypes, delegateEventTypes)) {
      result = toArray<TransactionActivity>(result).filter(e =>
        delegateEventTypes.includes(e.type!)
      );
    } else if (includesSomePatterns(resultTypes, transferEventTypes)) {
      result = toArray<TransactionActivity>(result).filter(e =>
        transferEventTypes.includes(e.type!)
      );
    } else {
      result = [
        (
          ...
          _,
          toArray<TransactionActivity>(result) as Record<string, unknown>[]
        ),
      ];
    }
  }
...
export const getSender = (
  ...
  return toArray(
    toArray<string | false>([
      ...
      f =>
        toArray<TransactionMessage>(messages).map(
          d => d[f as keyof TransactionMessage]
        )[0] ||
        (data as Record<string, unknown>)[`tx.body.messages.${f}`] ||
        toArray<TransactionActivity>(getActivities(data, assets)).find(
          d => (d as Record<string, unknown>)[f as string]
        )?.[f as keyof TransactionActivity]
    )
