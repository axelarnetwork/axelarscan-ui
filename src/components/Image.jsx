...
export function Image({ src, alt = '', className, ...props }) {
  return src && (
    <NextImage
      ...
      unoptimized
      className={clsx(className, isString(src) ? includesStringList(src, ['/immutable']) ? 'bg-white rounded-full' : includesStringList(src, ['/blast']) ? 'bg-zinc-900 rounded-full p-0.5' : '' : '')}
    />
  )
}
