...
export function getAddressPagePath(address: string, prefix: string, isVerifier: boolean | undefined): string {
...
export function setDefaultData(addresses: string[], data: Record<string, NameServiceEntry> | null) {
  let result = { ...data };
  addresses.forEach(a => {
    if (!result[a]) {
      result = { ...result, [a]: {} };
