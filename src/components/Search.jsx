  ...
  const onSubmit = async () => {
    ...
    if (type) {
      ...
      const { resolvedAddress } = {
        ...Object.values({ ...ens }).find(v =>
          equalsIgnoreCase(v.name, _input)
        ),
      };
      const spaceIDDomain = Object.values({ ...spaceID }).find(v =>
        equalsIgnoreCase(v.name, _input)
      );
      ...
      // transaction
        ...
        const gmpTotal = (
          await searchGMP({ txHash: _input, noRecover: true, size: 0 })
        )?.total;
        ...
        if (gmpTotal > 0) {
          ...
          const transfersTotal = (
            await searchTransfers({ txHash: _input, noRecover: true, size: 0 })
          )?.total;
    ...
    itsAssets && (
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="relative flex items-center">
          ...
          {searching && (
            <Spinner className="absolute right-0 top-0 mr-1 mt-1 !px-1" />
          )}
        </div>
      </form>
    )
