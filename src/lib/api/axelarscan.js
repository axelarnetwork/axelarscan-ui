...
const request = async (method, params, requestMethod = 'POST') => {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_AXELARSCAN_API_URL}/${method}${requestMethod === 'GET' ? objToQS(params) : ''}`,
    {
...
export const getTokensPrice = async params =>
  await request('getTokensPrice', params);
export const getInflation = async params =>
  await request('getInflation', params);
export const getNetworkParameters = async params =>
  await request('getNetworkParameters', params);
export const getBalances = async params => await request('getBalances', params);
export const getAccountAmounts = async params =>
  await request('getAccountAmounts', params);
