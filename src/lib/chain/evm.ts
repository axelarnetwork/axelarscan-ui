...
const createRPCProvider = (url: string, chainId?: string | number) =>
  new JsonRpcProvider(url, chainId ? toNumber(chainId) : undefined);

export const getProvider = (chain: string | undefined, chainsData: Chain[]) => {
  const { chain_id, deprecated, endpoints } = {
    ...getChainData(chain, chainsData),
  };
  const rpcs = toArray(endpoints?.rpc) as string[];
