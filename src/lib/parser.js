...
export const getIBCDenomBase64 = ibcDenom => {
  try {
    return Buffer.from(_.last(toArray(ibcDenom, { delimiter: '/' }))).toString(
      'base64'
    );
  } catch (error) {
    return ibcDenom;
...
export const getInputType = (string, chainsData) => {
  ...
  const regexMap = {
    ...
    cosmosAddress: Object.fromEntries(
      toArray(chainsData)
        .filter(d => d.prefix_address && d.prefix_address !== 'axelar')
        .map(d => [d.id, new RegExp(`${d.prefix_address}.*$`, 'igm')])
    ),
  };
  ...
  return (
    _.head(
      Object.entries(regexMap)
        .filter(([k, v]) =>
          k === 'cosmosAddress'
            ? Object.entries(v).findIndex(
                ([k, v]) =>
                  string.match(v) &&
                  string.startsWith(
                    chainsData.find(d => d.id === k)?.prefix_address
                  )
              ) > -1
            : string.match(v)
        )
...
export const toJson = string => {
  ...
  try {
    if (
      ...
      try {
        string = `{${split(string.substring(1, string.length - 1), {
          delimiter: ',',
        })
          .map(s =>
            split(s, { delimiter: ':' })
...
export const toHex = byteArray => {
  ...
  if (Array.isArray(byteArray))
    byteArray.forEach(
      byte => (string += ('0' + (byte & 0xff).toString(16)).slice(-2))
    );
...
export const getValuesOfAxelarAddressKey = data =>
  Object.entries({ ...data })
