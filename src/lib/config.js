  ...
  return (
    toArray(chainsData).find(d => {
      ...
      return (
        ...
        (!exact &&
          _.concat(keys, d.prefix_address, d.prefix_chain_ids)
            .filter(p => isString(p))
            .findIndex(p => chain.startsWith(p)) > -1) // check prefix
      );
...
export const getITSAssetData = (asset, assetsData) => {
  ...
  return toArray(assetsData).find(d =>
    find(asset, _.concat(d.id, d.symbol, d.addresses, d.address))
  );
};
